...
var vue = new Vue({
  ...
  methods: {
    sameAsAbove: function sameAsAbove(event) {
    ...
    defaultImage: function defaultImage(_defaultImage, uploadedImage, event) {
      var img = $('<img>');
      if (event.target.checked) {
        $('#listing-image').html(img.attr('src', _defaultImage));
      } else {
...
$(document).on('keydown', '.phoneNumber', function (event) {
  ...
  if (event.keyCode >= 48 && event.keyCode <= 57 || event.keyCode >= 96 && event.keyCode <= 105 || event.keyCode == 8 || event.keyCode == 9 || event.keyCode == 107 || event.keyCode == 187) {} else {
    event.preventDefault();
  }
  if ($(this).val().indexOf('+') !== -1 && (event.keyCode == 107 || event.keyCode == 187)) event.preventDefault();
});
